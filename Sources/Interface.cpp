...
AVSValue __cdecl Create_MDegrainN(AVSValue args, void*, IScriptEnvironment* env)
{
  ...
  return new MDegrainN(
    ...
    args[27].AsInt(0), // SEB param - soft edges blending block' border width in samples, must be even number (?), less than half block size (assumimg square block)
    // fixme: out32
    env
  );
}
...
AvisynthPluginInit3(IScriptEnvironment* env, const AVS_Linkage* const vectors) {
  ...
  env->AddFunction("MDegrainN", "ccci[thSAD]i[thSADC]i[plane]i[limit]f[limitC]f[thSCD1]i[thSCD2]i[isse]b[planar]b[lsb]b[thsad2]i[thsadc2]i[mt]b[out16]b[wpow]i[adjSADzeromv]f[adjSADcohmv]f[thCohMV]i[MVLPFCutoff]f[MVLPFSlope]f[MVLPFGauss]f[thMVLPFCorr]i[UseSubShift]i[SEB]i", Create_MDegrainN, 0);
